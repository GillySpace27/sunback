...
class SunbackMovie:
    ...
    def init_or_skip(self, ii):
        ...
        elif self.params.do_304():
            ii = 5

        self.this_name = self.wavelengths[ii]

        # if self.this_name in ['0193', '0304', '0171']:
        #     return True
        ...
        self.image_folder = self.save_path + "\\renders\\"
        self.movie_folder = self.save_path + "\\..\\movies\\"
    ...
    def space_to_fits(self):
        ...
        if self.params.download_images():
            if self.search_for_images_fido():
                self.parse_fido_result()
                self.download_images_fido()
    ...
    def search_for_images_fido(self):
        ...
        print(">Searching for Science Images from {} to {}...".format(self.earlyString, self.nowString), flush=True)

        # Search for records from the internet
        self.fido_result = Fido.search(a.Time(self.early, self.now), a.Instrument('aia'),
                                       a.Wavelength(int(self.this_name) * u.angstrom),
                                       a.Sample(self.params.cadence())) #, a.vso.Provider('jsoc'))
    ...
    def parse_fido_result(self):
        """Examine the search results"""

        first_result = self.fido_result.get_response(0)[0]
        last_result = self.fido_result.get_response(0)[-2]

        time_start = first_result.time_range().start
        time_end = last_result.time_range().end

        begin_time = self.parse_time_string_to_local(''.join(i for i in time_start.iso if i.isdigit()), 2)[0]
        end_time = self.parse_time_string_to_local(''.join(i for i in time_end.iso if i.isdigit()), 2)[0]
        ...
        self.name = first_result.blocks[0]['wave']['wavemin']
        
        while len(self.name) < 4:
            self.name = '0' + self.name

    def download_images_fido(self):
        print("       Search Found {} Images {}...".format(self.fido_num, self.extra_string), flush=True)
        try:
            Fido.fetch(self.fido_result, path=self.save_path,
    ...
    #         fn = file_to_get.fileid.casefold()
    #         if fn in self.already_downloaded:
