...
class SRNRadialFiltProcessor(Processor):
    ...
    def __init__(self, fits_path=None, in_name=-1, orig=False, show=False, verb=False):
        ...
        super().__init__()
        # super().set_function(self.do_fits_function)
        # Parse Inputs
        ...
        self.do_orig = orig
        self.center = None
        self.original = None# np.empty((10,10,), dtype=float)
        self.changed = None# np.empty((10,10,), dtype=float)
...
#  ##  From SRNRadialFiltProcessor Movie ##
# class ModifyMovie():
#     def execute(self):
#         if self.resume:
...
#     #         SRNRadialFiltProcessor(save_path, resolution=self.params.resolution())
#     #     # Sonify the data
...
#
#         if self.renew_mask or self.params.reprocess_mode() == 'r':
...
#
#         if type(reprocess_mode) in [float, int]:
#             mask_num = reprocess_mode
#         elif 'y' in reprocess_mode:
#             mask_num = 1
#         elif 'n' in reprocess_mode:
#             mask_num = 2
#         else:
#             if 'r' in reprocess_mode:
#                 if len(reprocess_mode) < 2:
#                     reprocess_mode += 'a'
#
#             if 'a' in reprocess_mode:
#                 top = 8
#                 btm = 1
#             elif 'h' in reprocess_mode:
#                 top = 6
#                 btm = 3
#             elif 'd' in reprocess_mode:
#                 top = 8
#                 btm = 7
#             elif 'w' in reprocess_mode:
#                 top = 2
#                 btm = 1
#             else:
#                 print('Unrecognized Mode')
...
def test_all(test_path="data/0171_MR.fits", show=True):
    ...
    test_mod = SRNRadialFiltProcessor(show=show)
    print("Success", flush=True)
    print("    Input String Method...", end='')
    test_mod2 = SRNRadialFiltProcessor(test_path, show=show)
    print("Success", flush=True)
    print("    Input Array Method...", end='')
    # image, image_data = load_best_fits_field(test_path)
    # test_mod3 = SRNRadialFiltProcessor(image, image_data, show=show)
    print("Success", flush=True)
...
if __name__ == "__main__":
    ...
    # in_object = in_object / np.mean(in_object)
    
    # in_object = in_object**(1/2)
    # in_object = np.log(in_object)
    
    # in_object = self.normalize(in_object, high=85, low=5)
...
class RadialFiltProcessor(Processor):
    ...
    #     filter = self.prep_radial_filter(wave)
    #
    #     pass
    #
    # def prep_radial_filter(self, wave):
    #
    #     pass
    #
    #
    # def __init__(self, fits_path, in_field=-1, orig=False, show=False, verb=False):
    #     """Initialize the main class"""
    #
    #     # Parse Inputs
    #     self.fits_path = fits_path
    ...
    #         get_field = self.in_field
    #
    #     frame, wave, t_rec, center = load_best_fits_field(fits_path, field1=get_field)
    ...
    #
    #     # Filter the regions separately
    #     reprocess_mode = 'nearest'
    #     low_max_filt = savgol_filter(self.low_max, lWindow, rank, reprocess_mode=reprocess_mode)
    #     mid_max_filt = savgol_filter(self.mid_max, mWindow, rank, reprocess_mode=reprocess_mode)
    #     # mid_max_filt = savgol_filter(mid_max_filt, mWindow, rank)
    #     # mid_max_filt = savgol_filter(mid_max_filt, mWindow, rank)
    #     # mid_max_filt = savgol_filter(mid_max_filt, mWindow, rank)
    #
    #     high_max_filt = savgol_filter(self.high_max, hWindow, rank, reprocess_mode=reprocess_mode)
    #
    #     low_min_filt = savgol_filter(self.low_min, lWindow, rank, reprocess_mode=reprocess_mode)
    #     mid_min_filt = savgol_filter(self.mid_min, mWindow, rank, reprocess_mode=reprocess_mode)
    #     high_min_filt = savgol_filter(self.high_min, hWindow, rank, reprocess_mode=reprocess_mode)
    #
    #     # Fit the lowest region with a polynomial to make it much smoother
    #     degree = 5
    #     p = np.polyfit(self.low_abs, low_max_filt, degree)
